...
module.exports = {
  ...
  importFastBootDependencies: function(app) {
    ...
    app.import(vendor + '/fastboot-mathjs.js');
  },
  ...
  getConfig: function() {
    var projectConfig = ((this.project.config(process.env.EMBER_ENV) || {}).mathjs || {});
    var mathjsPath = path.dirname(require.resolve('mathjs'));

    var config = defaults(projectConfig, {
      mathjsPath: mathjsPath
    });
  ...
  treeForBrowserVendor: function(vendorTree) {
    ...
    trees.push(new Funnel(options.mathjsPath, {
      destDir: 'mathjs',
